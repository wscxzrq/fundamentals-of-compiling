...
> 有穷自动机分为`确定的有穷自动机`（Deterministic Finite Automata，`DFA`）和`不确定的有穷自动机`（Nondeterministic Finite Automata,`NFA`）两类。确定的有穷自动机是指在一个状态下输入一个符号，状态转换到`唯一的下一个状态`（又称`后继状态`）；不确定的有穷自动机是指在一个状态下输入一个符号，可能`有两个或两个以上的后继状态`。
